    ...
    <div class="row my-4 flex" v-if="orderSummary">
            ...
            <div class="card border-0 rounded-3 flex-grow">
                <div class="card-body relative flex gap-2 flex-wrap flex-col">
                    <div class="inline-flex justify-between items-start">
                        ...
                        <span 
                            :style="{ backgroundColor: getOrderStatus(orderSummary?.order?.status)?.bgColor, color: getOrderStatus(orderSummary?.order?.status)?.color }"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                        <InfoDisplay label="Start Date" :value="formatDateAndTime(orderSummary.order.start_date)
                                ?.formattedDate
                            " />
                        <InfoDisplay label="End Date" :value="formatDateAndTime(orderSummary.order.end_date)
                                ?.formattedDate
                            " />
                        <InfoDisplay label="Order Type" :value="getOrderType(orderSummary.order.type)" />
                        <InfoDisplay label="Payment Status" value="Paid" />
                        <InfoDisplay label="Order Amount" :value="formatWithCommas(orderSummary.order.total_budget)" />
                    </div>
                    ...
                    <div>
                        <SummaryCard title="Customer Summary" :description="orderSummary.order.description"
                            readMoreLink="#" />
                    </div>
                </div>
            </div>
        ...
        <div class="col-md-3 flex flex-col gap-2">
            ...
            <div v-if="orderSummary.order.address" class="h-full flex-1">
                ...
                }" :address="orderSummary.order.address.address" buttonText="Open Map" />
            </div>
        </div>
        ...
        <div class="col-md-3 flex flex-col">
            ...
            <DescriptionCard v-if="orderSummary?.additional_terms" title="Additional terms" :content="orderSummary.additional_terms"
...
import { formatDateAndTime, formatWithCommas, getOrderType } from "../../utils/helper";
