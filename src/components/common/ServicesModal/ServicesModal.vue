<template>
  ...
  >
    <div class="relative overflow-y-auto" :style="modalStyles">
      <div class="flex flex-row items-center justify-between !bg-light-lilac rounded-tl-2xl rounded-tr-2xl px-6 py-2">
        <h2 class="!text-base !font-semibold text-dm-blue m-0">Edit/Add Service</h2>
        <button @click="closeModal" class="text-dm-blue hover:text-black !text-2xl font-semibold" type="button" aria-label="Close">
        ...
        <div class="flex flex-col !bg-white-100 rounded-bl-2xl rounded-br-2xl py-3 shadow-soft-blue space-y-4">
          <div class="px-6 flex flex-col gap-3">
            ...
            <InputField label="Unit Price" name="unit_price" 
              :secondValue="quantity" v-model="unit_price" v-bind="unit_priceAttrs" type="number" currency="AED" />
            <InputField label="Quantity" name="quantity" 
              :secondValue="unit_price" v-model="quantity" v-bind="quantityAttrs" type="number" currency="AED" />
            <InputField label="Total" name="total" 
              v-model="total" v-bind="totalAttrs" type="number" disabled currency="AED" />
            <DatePickerField name="delivery_time" label="Select Delivery Time" 
              placeholder="Pick a date" v-model="delivery_time" v-bind="delivery_timeAttrs" />
          </div>
          <div class="flex justify-end gap-3 border-t-[1.5px] py-6 border-solid border-soft-pastel-blue px-6">
...
<script setup>
import { ref, defineEmits, defineProps, computed, onMounted, onUnmounted, watch, watchEffect } from "vue";
...
const schema = yup.object({
  unit_price: yup.number().typeError("Unit Price must be a number").required("Unit Price is required").positive("Unit Price must be greater than zero"),
  quantity: yup.number().typeError("Quantity must be a number").required("Quantity is required").integer("Quantity must be a whole number"),
