  ...
  <div class="bg-white border-0 p-3 rounded rounded-4 shadow-sm">
    <div class="flex flex-col justify-between gap-3 relative">
      ...
      <!-- <InfoDisplay
        ...
        :value="
          getLeadTime(data.delivery_time)?.formattedDate +
          ' ' +
          getLeadTime(data.delivery_time)?.formattedTime
        "
      />
      <InfoDisplay
        label="Procurement/lead time"
...
<script setup>
import { defineProps, defineEmits, computed, watch, toRaw } from "vue";
