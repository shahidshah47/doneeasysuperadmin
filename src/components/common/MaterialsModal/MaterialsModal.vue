...
<script setup>
import {ref, computed, onMounted, onBeforeUnmount, defineProps, defineEmits, watch, getCurrentInstance} from "vue";
...
watch(
  ...
  (newData) => {
    if (newData) {
      ...
      setFieldValue("delivery_time", newData.delivery_time || newData.procurement_time);
