<template>
  <div
    class="flex justify-between rounded-lg items-center relative w-full pt-4"
  >
    <div class="text-gray-600 text-sm">
      <span class="text-sm !text-grayColor font-normal leading-5">
        Showing
      </span>
      <span class="text-sm !text-dm-blue font-semibold leading-5">{{
        start
      }}</span>
      ...
      <span class="text-sm !text-grayColor font-normal leading-5 mr-2">
        Entries</span
      >

      <div class="inline-block relative">
        <select
          class="bg-white rounded-md !w-16 z-50 focus:outline-none focus:ring-0 cursor-pointer"
          @change="(event) => selectEntry(event)"
        >
          <option
            v-for="option in entriesOptions"
            :key="option"
            :selected="option === selectedEntries"
            :class="[
              'rounded-sm text-sm text-vivid-purple cursor-pointer font-semibold !outline-none !focus:ring-0 hover:bg-light-lilac px-2 py-1',
    ...
    <div class="flex gap-2 items-center">
      <button
        ...
        @click="prevPage"
        class="rounded-md text-sm font-normal px-3 py-2"
        :class="
          currentPage === 1
      ...
      <button
        ...
        class="text-sm !font-semibold !rounded-md leading-5 px-3 py-2"
        :class="
          currentPage === page
...
const totalPages = computed(() =>
  Math.ceil(props.totalEntries / props.perPage)
);
const start = computed(() => (currentPage.value - 1) * props.perPage + 1);
const end = computed(() =>
  Math.min(currentPage.value * props.perPage, props.totalEntries)
);
