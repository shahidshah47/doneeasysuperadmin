  ...
  <div class="p-4 bg-white-100 rounded-xl min-h-[300px]">
    <div class="max-h-[300px] overflow-y-auto">
      <table class="w-full text-left border-collapse">
        ...
        <tbody>
          <template v-if="props.requests.length > 0">
            <tr
...
const appointmentId = computed(
  () => route.params.appointmentId || "default_id"
);

function navigateToDetails(req) {
  emit("viewDetail", [true, req.id]);
  router.push(
    `/super-admin/company-details/${companyId.value}/appointment/${appointmentId.value}/reoccurring/details`
  );
}
</script>
