...
const fetchData = async (id, page = 1, perPage = null, search = "") => {
  loading.value = true;
  try {
    let url = `/superadmin/user/site-surveys?all=1&user_id=${route.params.employeeId}&status=${id}&page=${page}&search=${search}`;
    if (perPage) {
      url = `/superadmin/user/site-surveys?all=1&user_id=${route.params.employeeId}&status=${id}&page=${page}&per_page=${perPage}`;
    }
    if (perPage && search !== "") {
      url = `/superadmin/user/site-surveys?all=1&user_id=${route.params.employeeId}&status=${id}&page=${page}&per_page=${perPage}&search=${search}`;
    }
    ...
    if (response?.status === 200) {
      ...
      surveysData.value = data?.map((item) => convertSurveyEmployeeData(item));
    }
  } catch (err) {
...
const handleDelete = async (id) => {
  try {
    const response = await api.post(
      `/superadmin/user/site-survey/${id}/delete`
    );
