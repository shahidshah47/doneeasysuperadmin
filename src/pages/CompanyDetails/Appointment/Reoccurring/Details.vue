<template>
  <div class="row my-4 flex" v-if="!isReqDetails">
    <div class="col-md-6">
      ...
      <RequestTable
        title="Order Details"
        :requests="requests"
        @viewDetail="(status, id) => handleViewDetails(status, id)"
      />
    </div>
    <div class="col-md-6">
        ...
        <div class="card-body relative flex gap-2 flex-wrap flex-col">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
            <InfoDisplay
              label="Date"
              :value="
                formatDateAndTime(appointDetails.delivery_date)?.formattedDate
              "
            />
            <InfoDisplay
              label="Time"
  ...
  <div class="row my-4 flex" v-if="requestDetails">
      ...
      <div class="card border-0 rounded-3 flex-grow">
        <div class="card-body relative flex gap-2 flex-wrap flex-col">
          <div class="inline-flex justify-between items-start">
            ...
            <span
              :style="{
                backgroundColor: getRequestStatus(requestDetails.status)
                  .bgColor,
                color: getRequestStatus(requestDetails.status).color,
              }"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
            <InfoDisplay
              label="Request Date"
              :value="
                formatDateAndTime(requestDetails.start_date)?.formattedDate
              "
            />
            <InfoDisplay
              label="Request Time"
              :value="
                formatDateAndTime(null, requestDetails.time)?.formattedTime
              "
            />
            <InfoDisplay
              label="Order Type"
    ...
    <div class="col-md-3 flex flex-col gap-2">
      <div v-if="requestDetails.associates?.length > 0" class="h-full flex-1">
        <div class="d-flex">
...
const handleSubmit = async (values) => {
  ...
  if (values?.id) {
    const data = {
      ...values,
      item_no: values.item_title,
      procurement_time: formatDateAtQuote(values.delivery_time),
    };
    delete data.item_title;
    console.log(data, "data updated");
    response = await api.post(
      "/superadmin/user/appointment/request/" +
...
const fetchRequestDetails = async (id) => {
  try {
    const response = await api.get(
      "/superadmin/user/appointment/request/" + id
    );
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (data) {
        ...
        user.value = isVendorOrAdmin.value
          ? appointDetails?.value?.user
          : offerUser || appointmentUser;
