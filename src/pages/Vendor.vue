...
const columns = ref([
    ...
    { field: "verticalsSubscribed", header: "List of Verticals Subscribed", class: "w-40" },
    ...
    <div>
        ...
        <div class="p-4 bg-white rounded-3 shadow relative">
            ...
            >
                ...
                <template #companyName="{ data }">
                    <div class="flex items-center gap-2">
                        <img :src="data.companyName.logo" alt="Company Logo" class="min-w-10 max-w-10 min-h-10 max-h-10 w-full rounded-xl object-cover" />
                        <span class="font-semibold text-dm-blue">{{ data.companyName.name }}</span>
                    </div>
                </template>

                <template #fullName="{ data }">
                    <div class="flex items-center gap-2">
                        <img :src="data.fullName.profilePicture" alt="Company Logo" class="min-w-10 max-w-10 min-h-10 max-h-10 w-full rounded-xl object-cover" />
                        <span class="font-semibold text-dm-blue">{{ data.fullName.name }}</span>
                    </div>
                </template>
                ...
                <template #contact="{ data }">
                    <div class="w-max flex flex-col flex-wrap gap-0">
                        <span class="font-semibold text-dm-blue">{{ data.contact.mobile }}</span>
                        <span class="text-primary font-semibold">{{ data.contact.email }}</span>
                    </div>
                </template>
                ...
                <template #totalSpending="{ data }">
                    <div class="flex gap-2 items-center">
                        <span class="text-primary font-semibold text-sm">AED</span>
                        <span class="text-2xl font-bold">{{ data.totalSpending ?? 0 }}</span>
                    </div>
                </template>

                <template #status="{ data }">
                    <Dropdown v-model="data.status" :options="statusOptions" 
                        @change="updateStatus(data)" :style="getStatusClass(data.status)" 
                        class="p-dropdown-sm font-bold"></Dropdown>
                </template>

                <template #verticalsSubscribed="{ data }">
                    <span class="px-3 py-2 rounded-xl font-semibold text-[#0E0D35] bg-light-lilac">{{ data.verticalsSubscribed }}</span>
                </template>
            ...
            <Paginator :rows="pagination.perPage" :totalRecords="pagination.total">
                <template #container="{ first, last, page, pageCount, rows, firstPageCallback, lastPageCallback, rowChangeCallback, prevPageCallback, nextPageCallback, totalRecords }">
                    <Pagination 
